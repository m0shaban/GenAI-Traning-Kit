...
class PasswordManager {
  ...
  init() {
    ...
    const delay = Number.isFinite(Number(delayAttr))
      ? Number(delayAttr)
  ...
  verifyPassword() {
    ...
    const expected = DynamicPasswordGenerator.normalize(
      CONFIG.CONTENT_PASSWORD()
    );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const copyProtection = isCopyProtectionEnabled()
    ? new CopyProtection()
    : null;
